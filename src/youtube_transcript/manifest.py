...
MANIFEST = {
    ...
    "skill": {
        "name": "yt-transcript",
        "description": (
            "Fetch the transcript of a YouTube video from the command line. Use when a "
            "prompt carries a YouTube URL or an 11-character video ID and asks for the "
            "transcript, the captions, what was said in the video, a summary of the video, "
            "a caption track in a particular language, or a translated transcript."
        ),
    },
    "install": {
        ...
        "install_without_uv": f"pip install git+{REPO}",
    },
    "conventions": [
...
def render_skill():
    """Render the manifest as a SKILL.md an agent can be installed with."""
    m = MANIFEST
    out = [
        "---",
        f"name: {m['skill']['name']}",
        f"description: {m['skill']['description']}",
        "---",
        "",
        f"# {m['tool']}",
        "",
        f"{m['summary']} Version {m['version']}. This file is generated by "
        "`yt-transcript describe --skill`; edit the manifest, not the copy.",
        "",
        ...
        m["install"]["install_without_uv"],
        "```",
        "",
        "Both need network access to GitHub. If neither install works, say so and stop; "
        "there is no offline path to a transcript.",
        "",
        "## How it behaves",
        "",
    ]
    ...
    out += [
        ...
        "",
        "The exit code says what went wrong and the message on stderr says what to do "
        "next. Read it before you retry with different arguments.",
        "",
    ]
